...
for result in searchCard_elements:
   ...
   if result:
      ...
      data.append([brand, model, year, price, location])

# Écrire les données dans un fichier CSV
with open("file.csv", "w", newline="") as fd:
   writer = csv.writer(fd)
   writer.writerow(["Marque", "Modèle", "Année", "Prix", "Emplacement"]) # Écrire les en-têtes
   for row in data:
      writer.writerow(row)

print("Données écrites dans le fichier file.csv.")